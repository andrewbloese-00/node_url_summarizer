 ...
 */
async function summaryCompletion(input,temperature=0.8,format="tl;dr like summary in the form of a paragraph"){
    try {
        const messages = [ 
            { role: "system", content: SYS_MSG },
            { role: "user", content: `Summarize the following text in a consise manner. It should formatted like: ${format}. Text = ${input}`}
        ]
 ...
 */
async function summarize(text = "",temperature=0.1,format="tl;dr like summary in the form of a paragraph"){
    //returns an array of chunks that are within token limits to prompt summaries with
   const chunked = chunkText(text)
   if(chunked.length === 1) { //base case 
